...
// ... (todas as suas outras funções como filtrarProcessos, ordenarProcessos, etc. continuam aqui, sem alterações)
function buscaVazia() {
...
function filtrarProcessos() {
    ...
    return processosOriginais.filter(item => {
        const processo = {
            ...
            reclamado: Array.isArray(item.reclamado) ? item.reclamado.join(', ') : (item.reclamado || item.nome_reclamado || '-'),
...
function renderizarTabela() {
    ...
    pagina.forEach(processo => {
        ...
        const dataFormatada = dataObj && !isNaN(dataObj) ? dataObj.toLocaleDateString('pt-BR') : '-';
        ...
        tr.innerHTML = `
            ...
            <td><span class="status-badge status-${statusFormatado.classe}">${statusFormatado.texto}</span></td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CÓDIGO ADICIONADO ---
    // Adiciona um listener de clique na tabela inteira
    const tableBody = document.getElementById('table-body');
    tableBody.addEventListener('click', function(event) {
        // Procura pelo botão de visualização mais próximo do local do clique
        const viewButton = event.target.closest('.action-view');

        if (viewButton) {
            const processoId = viewButton.dataset.id;
            // Redireciona para a página de detalhes, passando o ID como parâmetro na URL
            window.location.href = `../Detalhes/detalhes.html?id=${processoId}`;
        }
    });
